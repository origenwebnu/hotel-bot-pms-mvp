...
$COMPOSE_CMD exec -T api sh -c "$NODE_CMD -e \"
...
echo "  sed -i 's|^SMTP_FROM=.*|SMTP_FROM=\"BookiChat <noresponder@janpublicidad.com>\"|' .env"
echo "  docker compose -f docker-compose.yml -f docker-compose.ssl.yml up -d --force-recreate api"
