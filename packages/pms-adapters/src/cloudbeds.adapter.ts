...
export class CloudbedsAdapter implements PmsAdapter {
  async getAvailability(
    ...
    try {
      const baseUrl =
        credentials.base_url ?? 'https://api.cloudbeds.com';
      ...
      const response = await fetch(
        `${baseUrl}/api/v1.1/getAvailableRoomTypes?${params}`,
        {
          headers: {
      ...
      const data = (await response.json()) as CloudbedsAvailabilityResponse;
  ...
  async holdRoom(
    ...
    const data = (await response.json()) as CloudbedsHoldResponse;
  ...
  async confirmReservation(
    ...
    const data = (await response.json()) as { reservationID: string; confirmationCode?: string };
    return {
  ...
  async validateCredentials(credentials: PmsCredentials): Promise<boolean> {
    try {
      const baseUrl = credentials.base_url ?? 'https://api.cloudbeds.com';
      const response = await fetch(
        `${baseUrl}/api/v1.1/getHotels?propertyIDs=${credentials.property_id}`,
        {
  ...
  private mapRooms(data: CloudbedsAvailabilityResponse): StandardRoomAvailability[] {
  ...
  private fallbackResult(code: 'PMS_UNAVAILABLE' | 'PMS_TIMEOUT' | 'PMS_AUTH_ERROR'): AvailabilityResult {
    return {
      rooms: [],
      pms_source: 'cloudbeds',
      queried_at: new Date().toISOString(),
...
interface CloudbedsAvailabilityResponse {
  data?: Array<{
    ...
    roomTypePhotos?: Array<{ photoURL: string }>;
  }>;
}

interface CloudbedsHoldResponse {
